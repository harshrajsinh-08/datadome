...
class DataImputer:
    ...
    def impute_missing_values(self,df: pd.DataFrame, column_dtypes: dict, eps: float = 0.5, min_samples: int = 5, n_components: int = 2) -> pd.DataFrame:
        ...
        self.outlier_report = {
            ...
            'outlier_percentage': round(outliers_removed / len(df) * 100, 2),
            'outlier_indices': outlier_indices
        }
        print(f"Outlier detection: removed {outliers_removed} outliers ({self.outlier_report['outlier_percentage']}%)")
