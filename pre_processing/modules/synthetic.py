...
def generate_synthetic_data(df, output_dir = '.', synthetic_fraction=0.2):
    ...
    """
    try:
        print(f"Starting synthetic data generation for {len(df)} rows...")
        num_synthetic = int(len(df) * synthetic_fraction)
        print(f"Will generate {num_synthetic} synthetic rows...")
        ...
        for column in df.columns:
            ...
            if column_type in ['object', 'boolean']: 
        ...
        jpath = f"{output_dir}/profiling_report.json"
        ...
        combined_data.to_csv("output/synthetic.csv")
        print("Synthetic data generation completed successfully!")
        
        return combined_data
    
    except Exception as e:
        print(f"Error in synthetic data generation: {str(e)}")
        print("Returning original data without synthetic augmentation...")
        return df