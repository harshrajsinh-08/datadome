...
def main(file_path, output_dir = 'output', gen_syn_data=False, target=None, task=None):
    ...
    # Apply user-defined rules from submitted_data.json
    try:
        original_df = rule_based_cleaning(original_df, "output/submitted_data.json")
        print("Applied user-defined cleaning rules")
    except Exception as e:
        print(f"Could not apply user rules: {e}")
        print("Proceeding without user-defined rules")
    
    # Detect missing values BEFORE any processing
    data_cleaner.detect_missing_values(original_df)
    
    # Process data types and duplicates
    ...
    # Impute missing values and detect outliers
    ...
    if gen_syn_data: 
        ...
        print("Synthetic data generation skipped (checkbox not checked)")
